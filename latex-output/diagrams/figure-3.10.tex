...
\begin{tikzpicture}[node distance=0pt]
  ...
  \draw[flow] (step2) -- (det);
  % detected? Yes -> seated check ; No -> check USB (a dead-end fix-it step)
  \draw[flow] (det)   -- node[elabel,above]{Yes} (seat);
  \draw[flow] (det)   -- node[elabel,right]{No} (usb);

  % seated? Yes -> run calibration ; No -> back up to positioning
  \draw[flow] (seat) -- node[elabel,right,xshift=3pt]{Yes} (step3);
  \draw[flow] (seat.north) |- ($(step2.north)+(0,1.4)$)
        node[elabel,pos=0.25,above,yshift=3pt]{No} -- (step2.north);

  % run calibration -> calibrated? ; No loops back, Yes -> validation
  \draw[flow] (step3) -- (calok);
  \draw[flow] (calok.west) -- ++(-0.9,0) node[elabel,pos=0.6,above]{No} |- (step3.west);
  \draw[flow] (calok) -- node[elabel,right,xshift=3pt]{Yes} (step4);
