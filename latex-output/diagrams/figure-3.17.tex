...
\begin{tikzpicture}
  ...
  % extra room on the left of the fit so the rotated label sits inside
  \coordinate (loopL) at ($(keep.west)+(-0.55,0)$);
  \node[draw=dioBlueS, line width=1pt, fill=dioGrayF, fill opacity=0.45,
        rounded corners=14pt, inner xsep=10pt, inner ysep=12pt,
        fit=(present)(keep)(mean)(loopL)] (loop) {};
  ...
  \node[rotate=90, font=\small\bfseries, anchor=north]
        at ($(loop.west)+(0.08,0)$) {Repeat for all 5 targets};

  \draw[flow] (present) -- (keep);
  \draw[flow] (keep) -- (mean);

  % ---- Post-loop metric computation --------------------------------------
  \node[gray] (dist)  at (12.7,1.2)  {Measure viewing distance};
  \node[gray] (err)   at (12.7,-1.2) {Compute on-screen error};
